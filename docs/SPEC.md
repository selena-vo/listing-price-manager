...
A web app for **homestay owners and homestay managers** — a "short-term rental price manager" — that lets them see the **price currently displayed** for their homestay on rental platforms such as Airbnb, Trip.com, Booking.com.

- Users can **customize the list of platforms** they track (add/remove platforms at any time).
- For each platform, users see the current price of their homestay on that platform — comparing all channels in one screen instead of opening each platform's app/tab separately.
...
- **Lin, 35 — homestay owner.** Owns 1–3 properties, lists them on 3–4 platforms, and wants to see all listing prices in one place without logging into each platform. Not very technical; needs a simple, obvious UI.
- **Nam, 28 — homestay manager.** Manages several homestays for owners and updates prices regularly; needs a quick board to check prices, spot missing/outdated values, and know the real earnings after each platform's commission and campaigns.
...
- [M] As a homestay owner, I want to **add my homestays**, so that I can track prices for each one.
- [M] As a homestay owner, I want to **add and remove platforms** (Airbnb, Booking.com, Trip.com, or custom), so that the app matches the platforms I actually use.
- [M] As a homestay owner, I want to **set the commission rate (hoa hồng) for each platform**, so that net estimates are correct.
- [M] As a homestay owner, I want to **create and activate one or more discount campaigns per platform** (e.g. "−10% autumn promo", "−5% weekday"), so that I know the price guests actually pay.
- [M] As a homestay owner, I want to **choose the discount combination rule for each platform** (sum / best / priority / sequential), so that the final price matches that platform's real rules.
- [M] As a homestay owner, I want to **set and update the price per night** for each homestay on each platform, so that the board always reflects my current listings.
- [M] As a homestay owner, I want to **see all my homestays and their prices on every platform in one dashboard**, including the **estimated net after commission and campaigns**, so that I can compare channels at a glance.
- [M] As a homestay manager, I want to **see when a price has not been set** on a platform, so that I know what needs updating.
- [N] As a homestay owner, I want a **history of price changes**, so that I can review how my prices evolved.
- [N] As a homestay owner, I want to **schedule campaigns with start/end dates**, so that they apply automatically.
- [N] As a homestay owner, I want to **sort/filter** the board by property or platform, so that I can focus on part of my portfolio.
- [N] As a homestay owner, I want **custom/free-form formulas** as a rule option, so that unusual platform rules can be represented exactly.

## 4. Screens & flows

- **S1 — Dashboard (price board):**
  - A table: rows = homestays, columns = platforms (the customized list). Each column header shows the platform name and its commission % (e.g. "Airbnb · 15%").
  ...
  - Empty cell → "—" means the price is not set for that homestay+platform; highlighted visually so it's obvious.
  - Actions: "Add homestay" (→ S3), "Add platform" (→ S4), click a price cell (→ S2), edit/delete a homestay.
...
- **S3 — Add/Edit homestay (modal or page):** fields: name (required), location (optional), notes (optional). Save → new row appears on the dashboard with all price cells empty.
...
- **Homestay** — id, name (text), location (text, optional), notes (text, optional), createdAt
- **ListingPrice** — id, homestayId → Homestay, platformId → Platform, pricePerNight (number, > 0), currency (text, e.g. "VND"), note (text, optional), updatedAt
  - Constraint: **one price per (homestay, platform) pair** — updating overwrites the previous value.
...
- List homestays **with all their prices** — one call returns the whole dashboard, including each platform's commission %, discount rule, and active campaigns (with priority order); create / update / delete a homestay
- Set or update the price for a (homestay, platform) pair; read a single price
...
- [ ] On the live URL: I can add a homestay, add a platform, set prices per night, and see everything in the dashboard.
...
- Multiple room types per homestay / multi-unit pricing — one price per homestay per platform (see Q3)
...
| 3 | Multiple rooms / room types per homestay? Price unit? | **One price per homestay per platform, per night** | ✔ |
...
| Aug 23, 2026 | **PO confirmed Q1–Q5** (manual entry, no login MVP with auth planned later, one price per homestay–platform, VND, no price history) → **spec v1.0 MVP baseline approved**; changelog consolidated | PO |
| Aug 23, 2026 | **Stack re-initialized on the Next.js SaaS Starter** (dev decision, PO informed): Next.js 15 + React 19 + Tailwind 4 + shadcn/ui + Drizzle ORM + **Postgres** (replaces SQLite) + JWT auth (ships with the starter — MVP keeps single workspace per Q2; PO to confirm keep vs strip) + Stripe (kept, unused by MVP). Domain code (rule engine, presets) and the DB schema for platforms/homestays/listing_prices/campaigns were added. Spec content unchanged | Dev |
